...
interface FormData {
  ...
  linkedin: string;
  discord: string;
  title: string; // Changed from jobTitle
  skills: string;
  description: string; // Changed from jobDescription
  portfolio: string;
  profileImageUrl?: string;
  bannerImageUrl?: string;
  jobBannerImageUrl?: string;
  emailVerified?: boolean;
  email?: string; // Changed from verifiedEmail
}
...
const ProfilePage = () => {
  ...
  const [jobBannerImage, setJobBannerImage] = useState<string | null>(null);
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>, type: 'profile' | 'banner' | 'jobBanner') => {
    ...
    if (file) {
      try {
        ...
        if (!file.type || !allowedTypes.includes(file.type)) {
          showToastAlert('Invalid file type. Only JPEG, PNG, GIF, and WebP are allowed.', 'error');
          return;
        }
        ...
        if (file.size > maxSize) {
          showToastAlert('File too large. Maximum size is 10MB.', 'error');
          return;
        }
        ...
        reader.onloadend = () => {
          if (type === 'profile') {
            ...
            setJobBannerImage(reader.result as string);
          }
        };
        ...
        setFormData(prev => ({
          ...prev,
          [type === 'profile' ? 'profileImageUrl' : type === 'banner' ? 'bannerImageUrl' : 'jobBannerImageUrl']: cloudUrl
        }));
        showToastAlert(`${type.charAt(0).toUpperCase() + type.slice(1)} image uploaded successfully!`, 'success');
      } catch (error) {
        console.error(`Error handling ${type} image upload:`, error);
        showToastAlert(`Failed to upload ${type} image`, 'error');
      }
    }
  };
    ...
    if (verificationCode === cachedOTP) {
      try {
        ...
        setFormData(prev => ({
          ...
          jobBannerImageUrl: userData?.jobBannerImageUrl || ''
        }));
        setProfileImage(userData?.profileImageUrl || null);
        setBannerImage(userData?.bannerImageUrl || null);
        setJobBannerImage(userData?.jobBannerImageUrl || null);
  ...
  const handleSaveChanges = async () => {
    ...
    try {
      ...
      const profileData = {
        ...
        jobTitle: formData.title, // Map to title
        skills: formData.skills ? formData.skills.split(',').map(s => s.trim()).filter(s => s) : [],
        jobDescription: formData.description, // Map to description
        portfolio: formData.portfolio,
        jobBannerImage: formData.jobBannerImageUrl
      };
      ...
      setFormData(prev => ({
        ...
        profileImageUrl: savedData.profileImage || '',
        bannerImageUrl: savedData.bannerImage || '',
        title: savedData.jobTitle || '',
        skills: savedData.skills ? savedData.skills.join(', ') : '',
        description: savedData.jobDescription || '',
        portfolio: savedData.portfolio || '',
        jobBannerImageUrl: savedData.jobBannerImage || ''
      }));
      setProfileImage(savedData.profileImage || null);
      setBannerImage(savedData.bannerImage || null);
      setJobBannerImage(savedData.jobBannerImage || null);
            ...
            {activeTab === 'job' && isEmailVerified && (
              <>
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    <label className="block text-sm text-gray-400 mb-1">Skills (Enter your top skill set, comma-separated)</label>
                    <input
                      type="text"
                      name="skills"
                      ...
                      onChange={handleInputChange}
                      className="w-full bg-[#1C1C1E] rounded-md p-3 text-white"
                      placeholder="e.g., blockchain, solidity, react"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <label className="inline-flex items-center gap-2 bg-[#1C1C1E] px-4 py-2 rounded-md cursor-pointer hover:bg-[#2C2C2E]">
                      ...
                      <input
                        type="file"
                        className="hidden"
                        onChange={(e) => handleImageUpload(e, 'jobBanner')}
                        accept="image/*"
                      />
                    </label>
                    {jobBannerImage && (
